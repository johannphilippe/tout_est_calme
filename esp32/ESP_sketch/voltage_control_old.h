...
struct voltage_control
{
    ...
    struct threshold_t
    {
        ...
        static constexpr float fast_peak = 0.3f;
        ...
        static constexpr float slow_peak = 0.05f;
        ...
        static constexpr float very_slow_peak = 0.0f;
        ...
        static constexpr float medium_peak = 0.1f;
    ...
    */
    uint16_t operator() (float destination, float source, float current)
    {
        ...
        {
            res -= current + direction * thresholds.very_near_step; // BUG: current is Hz not pwm ratio
        } else
        {
            // Do nothing
        }
